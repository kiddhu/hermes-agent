...
BOARD_COLUMNS: list[str] = [
    "triage", "todo", "scheduled", "ready", "running", "fenced", "blocked", "review", "done",
]
        ...
        if payload.status is not None:
            ...
            if s == "done":
                ...
                if current and current.status in ("blocked", "scheduled"):
                    ...
                    # routed through the public controlled writer.
...
def _set_status_direct(
    ...
    """Backward-compatible alias for the public controlled status writer.

    The direct ``tasks.status`` lifecycle SQL used to live here; it has moved
    into :func:`hermes_cli.kanban_db.set_task_status` so the dashboard no
    longer owns a status-writer bypass. ``update_task``/``bulk_update`` call
    the public writer directly. This private shim is retained only so
    pre-existing direct callers keep working unchanged.
    """
