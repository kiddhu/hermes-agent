...
# gate entry total on the authenticated creator identity (``created_by``,
# derived server-side from the session identity), so omitting every optional
...
# REV2 — self-declared actor identity sources (mirror of the kernel's
# SELF_DECLARED_ACTOR_SOURCES / BYPASS_SELF_DECLARED_ACTOR_IDENTITY). A receipt
# whose ``action_receipt_ref.actor_identity_source`` resolves into this set is
# rejected: actor identity must come from a canonical provider/runtime metadata
# source, never self-declared prose.
FACTORY_SELF_DECLARED_ACTOR_SOURCES = frozenset({
    "", "self", "self_declared", "self-declared", "raci_prose", "narrative",
    "agent_claim", "self_claim", "prompt", "assistant",
})

# REV2 — closed actor roles (mirror of the kernel's ACTOR_ROLES). The receipt's
# ``action_receipt_ref.actor_role`` must be one of these.
FACTORY_ACTOR_ROLES = frozenset({
    "implementation_commit_author",
    "pr_creator",
    "action_executor",
    "auditor",
    "merger_or_closer",
...
# Immutable authority for this module set: aion-governance PR #947, independently
...
AION_GOVERNANCE_AUTHORITY_PR = 947
AION_GOVERNANCE_AUTHORITY_HEAD = "339312bcdc9794fe996f28aadb3309a1d46b3b4e"
AION_GOVERNANCE_AUTHORITY_COMMIT = "256bf3e56869d7219b5712d45066e96ce4ec2c6d"
...
# exact PR #947 merge bytes at this content-addressed path, or explicitly point
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Creation-time explicit block** — ``create_task(initial_status=
      "blocked")`` parks a real external gate (financial/legal/property)
      directly in ``blocked`` to avoid the running→blocked race.  It
      records only a ``"created"`` event whose payload ``status`` is
      ``"blocked"`` — no ``"blocked"`` event.  Both fresh and legacy rows
      of this shape must stay blocked until an explicit ``unblock_task``
      (AION-RL2-CORE-01-R13, canonical incident t_e690dcc1).
...
def _canonical_completed_recovery_receipt(receipt: Any) -> Optional[dict[str, Any]]:
    """Return one strictly closed completed-audit recovery receipt."""
    ...
    if (
        receipt.get("review_outcome") != "REQUEST_CHANGES_EXACT_HEAD"
...
def _normalize_completed_pass_recovery_receipt(receipt: Any) -> Optional[dict[str, Any]]:
    ...
    present, unique and non-conflicting:
      ...
      * ``head`` | ``exact_head`` — when both keys are present their values
        must be byte-identical; when only one is present its value must be a
        non-null SHA.  A key that is present but null is a conflict, not an
        absence, so it fails closed.
      * ``review_outcome == "approved"`` | ``verdict == "PASS_EXACT_HEAD"`` —
        when both keys are present they must agree on an approved PASS; when
        only one is present its value must be the approved-PASS spelling.  A
        key that is present but null is a conflict, not an absence.

    Alias membership is decided by key presence (``key in receipt``), never by
    value truthiness, so a ``head: null`` alongside a populated ``exact_head``
    (and a ``review_outcome: null`` alongside a ``verdict``) fails closed
    ...
    has_head = "head" in receipt
    has_exact_head = "exact_head" in receipt
    if has_head and has_exact_head:
        if head != exact_head:
            return None
    elif has_exact_head:
    ...
    has_review_outcome = "review_outcome" in receipt
    has_verdict = "verdict" in receipt
    if has_review_outcome and has_verdict:
...
def _canonical_completed_recovery_payload(row: sqlite3.Row) -> Optional[dict[str, Any]]:
    ...
    if (
        ...
        or payload["verdict"] != "request_changes"
...
def _completed_recovery_history_is_non_authoritative(
    conn: sqlite3.Connection,
    *,
    author_task_id: str,
    author_profile: str,
    ...
    """Authenticate one terminal audit converted by the version-2 recovery path."""
    if (
        author_profile != FACTORY_REVIEW_AUTHOR_PROFILE
        or auditor_profile != FACTORY_REVIEW_AUDITOR_PROFILE
        or len(runs) != 1
        or runs[0]["status"] != "done"
        or runs[0]["outcome"] != "completed"
    ):
        return False
    ...
    payload = _canonical_completed_recovery_payload(recovery_row)
    review_run_id = int(runs[0]["id"])
    if (
        payload is None
        or payload["review_task_id"] != auditor_task_id
        or payload["review_run_id"] != review_run_id
        or payload["reason"] != runs[0]["summary"]
        or type(recovery_row["created_at"]) is not int
    ):
        return False
    try:
        metadata = json.loads(runs[0]["metadata"] or "{}")
    except (TypeError, ValueError):
        return False
    receipt, family = _closed_completed_audit_recovery_receipt(
        metadata, _COMPLETED_RECOVERY_RECEIPT_KEYS,
    )
    if (
        family != "top_level"
        or receipt != _canonical_completed_recovery_receipt(
    ...
    controller_row = conn.execute(
        ...
        "FROM tasks WHERE id = ?", (controller["task_id"],),
    ).fetchone()
    controller_runs = conn.execute(
        "SELECT id, profile, status, outcome, started_at, ended_at FROM task_runs "
        "WHERE task_id = ? ORDER BY id DESC", (controller["task_id"],),
    ).fetchall()
    identity_fields = (
        "current_run_id", "claim_lock", "claim_expires", "worker_pid",
        "worker_starttime", "fence_lineage", "fence_disposition",
    ...
    if (
        ...
        or controller_row["status"] not in {"done", "archived"}
        or controller_row["assignee"] != controller["profile"]
        or any(controller_row[field] is not None for field in identity_fields)
        or not controller_runs
        or int(controller_runs[0]["id"]) != controller["run_id"]
        or controller_runs[0]["profile"] != controller["profile"]
        or controller_runs[0]["status"] != "done"
        or controller_runs[0]["outcome"] != "completed"
        or controller_runs[0]["ended_at"] is None
        or type(controller_runs[0]["started_at"]) is not int
        or not (
            int(controller_runs[0]["started_at"])
            <= int(recovery_row["created_at"])
            <= int(controller_runs[0]["ended_at"])
        )
    ):
        return False
    if conn.execute(
...
def _historical_auditor_child_is_non_authoritative(
    ...
    *,
    ...
    Every run must be machine-bound to an earlier typed handoff and terminal
    REQUEST_CHANGES.  Two narrow historical variants are also recognized: the
    exact version-2 completed-audit recovery receipt, and one initial blocked
    precursor before a reused child's complete REQUEST_CHANGES rounds.
    """
    identity_fields = (
        "current_run_id", "claim_lock", "claim_expires", "worker_pid",
        "worker_starttime", "fence_lineage", "fence_disposition",
    ...
    if (
        ...
        or child["status"] not in {"todo", "done", "archived"}
    ...
    if child["status"] == "done":
        return _completed_recovery_history_is_non_authoritative(
            ...
            latest_handoff_event_id=latest_handoff_event_id,
            runs=runs,
        )
    if child["status"] not in {"todo", "archived"}:
        return False
    ...
    if archive_event_id is not None and archive_event_id > latest_handoff_event_id:
        allowed_post_ids.add(archive_event_id)
    ...
    for row in handoff_rows:
        ...
        if receipt.review_task_id == auditor_task_id:
            bound_handoffs.append(receipt)
    if not bound_handoffs:
        return False

    run_by_id = {int(row["id"]): row for row in runs}
    seen_run_ids: set[int] = set()
    precursor_seen = False
    request_changes_rounds = 0
    for index, prior_handoff in enumerate(bound_handoffs):
        next_event_id = (
            bound_handoffs[index + 1].event_id
            if index + 1 < len(bound_handoffs)
            else latest_handoff_event_id
        )
        if prior_handoff.event_id >= next_event_id:
            return False
        author_run = conn.execute(
            "SELECT profile, status, outcome, ended_at FROM task_runs "
            "WHERE id = ? AND task_id = ?",
        ...
        verdict_rows = conn.execute(
            ...
            (author_task_id, prior_handoff.event_id, next_event_id),
        ).fetchall()
        matching = []
        for verdict_row in verdict_rows:
            ...
            if payload["review_task_id"] == auditor_task_id:
                matching.append((verdict_row, payload))
        blocked = conn.execute(
            "SELECT id, run_id, payload FROM task_events WHERE task_id = ? "
            "AND kind = 'blocked' AND id > ? AND id < ? ORDER BY id",
        ...
        mirrors = conn.execute(
            ...
            "ORDER BY id",
            (auditor_task_id, review_run_id, prior_handoff.event_id, next_event_id),
        ).fetchall()
        if (
            review_run_id in seen_run_ids
            or run is None
            or payload["verdict"] != "request_changes"
            or run["status"] != "request_changes"
            or run["outcome"] != "request_changes"
            or len(mirrors) != 1
...
def _canonical_audit_receipt(
    ...
    historical_handoff_rows = conn.execute(
        "SELECT id, run_id, payload FROM task_events WHERE task_id = ? "
        "AND kind = 'review_handoff' AND id < ? ORDER BY id",
        (task_id, handoff.event_id),
    ).fetchall()
    historical_handoff_ids = set()
    for row in historical_handoff_rows:
        historical_handoff = _review_handoff_receipt_from_row(task_id, row)
...
def _strict_terminal_archive_is_authenticated(
    conn: sqlite3.Connection,
    task_id: str,
) -> bool:
    """Authenticate one orchestrator archive with no completed action receipt."""
    archive_rows = conn.execute(
        "SELECT id, run_id, payload, created_at FROM task_events "
        "WHERE task_id = ? AND kind = 'archived' ORDER BY id",
        (task_id,),
    ...
    if len(archive_rows) != 1 or len(auth_rows) != 1:
        return False
    archive_row, auth_row = archive_rows[0], auth_rows[0]
    try:
        archive_payload = json.loads(archive_row["payload"] or "{}")
        auth_payload = json.loads(auth_row["payload"] or "{}")
    except (TypeError, ValueError):
    ...
    if (
        ...
        or archive_payload["source"] != "kanban_archive"
        or archive_row["run_id"] is not None
        or auth_payload != {"version": 1}
        or auth_row["run_id"] is not None
        or int(auth_row["id"]) != int(archive_row["id"]) + 1
        or auth_row["created_at"] != archive_row["created_at"]
    ):
        return False
    if conn.execute(
    ...
    return all(
        ...
        and (row["status"], row["outcome"])
        in {("blocked", "blocked"), ("request_changes", "request_changes")}
        for row in runs
    )
...
def _terminal_request_changes_auditor_can_rearm(
    ...
    *,
    ...
    if (
        author is None
        or author["status"] != "running"
        or author["assignee"] != FACTORY_REVIEW_AUTHOR_PROFILE
        or author["current_run_id"] != expected_run_id
        or auditor is None
        or auditor["status"] != "done"
        or auditor["assignee"] != FACTORY_REVIEW_AUDITOR_PROFILE
        or any(auditor[field] is not None for field in identity_fields)
        or current_run is None
        or current_run["profile"] != FACTORY_REVIEW_AUTHOR_PROFILE
        or current_run["status"] != "running"
        or current_run["outcome"] is not None
        or current_run["ended_at"] is not None
        or conn.execute(
...
# ---------------------------------------------------------------------------
# Reviewed-author handoff recovery (GM/GM2 controller lane)
# ---------------------------------------------------------------------------

# Typed event kind minted by the reviewed-author resume operation. It is a
# controller-scoped recovery receipt, not a review verdict: it never infers
# PASS/REQUEST_CHANGES and never rewrites the immutable handoff/run bytes.
REVIEW_HANDOFF_RECOVERY_EVENT_KIND = "review_handoff_recovery"


def _review_handoff_recovery_receipt_from_row(
    row: sqlite3.Row,
) -> Optional[dict[str, Any]]:
    """Parse the one strictly typed reviewed-author recovery receipt."""
    try:
        payload = json.loads(row["payload"] or "{}")
    except (TypeError, ValueError):
        return None
    ...
    strict_types = (
        (payload["version"], int),
        (payload["author_task_id"], str),
        (payload["audit_task_id"], str),
        (payload["review_handoff_event_id"], int),
    ...
    if (
        payload["version"] != 1
        or not payload["author_task_id"].strip()
        or not payload["audit_task_id"].strip()
        or payload["author_task_id"] == payload["audit_task_id"]
        or payload["review_handoff_event_id"] <= 0
    ...
    signed = {
        "version": payload["version"],
        "author_task_id": payload["author_task_id"],
        "audit_task_id": payload["audit_task_id"],
        "review_handoff_event_id": payload["review_handoff_event_id"],
...
def _resume_reviewed_author(
    ...
    *,
    ...
    """Atomically resume one reviewed author whose direct audit child failed
    closed before a Native verdict.
    ...
    try:
        ...
        return None
    with write_txn(conn):
        # Idempotency / double-recovery guard on prior recovery receipts.
        for row in conn.execute(
            "SELECT id, run_id, payload FROM task_events WHERE task_id = ? "
            "AND kind = ? ORDER BY id",
            (author_task_id, REVIEW_HANDOFF_RECOVERY_EVENT_KIND),
        ).fetchall():
            receipt = _review_handoff_recovery_receipt_from_row(row)
            if receipt is None:
                # A malformed/forged recovery event already exists: cannot
                # prove idempotency, so fail closed.
                return None
            if (
                receipt["author_task_id"] == author_task_id
                and receipt["audit_task_id"] == audit_task_id
                and receipt["review_handoff_event_id"] == review_handoff_event_id
            ):
                return {**receipt, "event_id": int(row["id"])}
            # A different recovery already targeted this author: no double
            # recovery of the same author lineage.
            return None
        ...
        if author is None or audit is None:
            return None
        if not author["assignee"] or not audit["assignee"]:
            return None
        if author["assignee"] == audit["assignee"]:
            return None
        if (
            author["status"] != "review"
            or any(author[field] is not None for field in _REVIEW_IDENTITY_FIELDS)
        ):
            return None
        if (
            audit["status"] not in {"todo", "blocked", "triage"}
            or any(audit[field] is not None for field in _REVIEW_IDENTITY_FIELDS)
        ):
            return None
        if parent_ids(conn, audit_task_id) != [author_task_id]:
            return None

        handoff_row = conn.execute(
            "SELECT id, run_id, payload FROM task_events "
            "WHERE id = ? AND task_id = ? AND kind = 'review_handoff'",
            (review_handoff_event_id, author_task_id),
        ).fetchone()
        ...
        author_run = conn.execute(
            "SELECT id, status, outcome, ended_at FROM task_runs "
            "WHERE id = ? AND task_id = ?",
            (handoff.expected_run_id, author_task_id),
        ).fetchone()
        if (
            author_run is None
            or author_run["status"] != "review_required"
        ...
        if latest_author_run != handoff.expected_run_id:
            return None
        ...
        if conn.execute(
            "SELECT 1 FROM task_events WHERE task_id = ? AND kind = 'review_verdict' "
            "AND id > ? LIMIT 1",
            (author_task_id, review_handoff_event_id),
        ).fetchone() is not None:
            return None
        ...
        _append_event(
            conn, author_task_id, REVIEW_HANDOFF_RECOVERY_EVENT_KIND,
            {**core_payload, "receipt_sha256": receipt_sha256},
        )
        event_id = int(conn.execute("SELECT last_insert_rowid()").fetchone()[0])
        return {**core_payload, "receipt_sha256": receipt_sha256, "event_id": event_id}


def resume_reviewed_author(
    conn: sqlite3.Connection,
    *,
    author_task_id: str,
    audit_task_id: str,
    ...
    """Public fail-closed wrapper for the reviewed-author resume transaction."""
    try:
        return _resume_reviewed_author(
            conn,
            author_task_id=author_task_id,
            audit_task_id=audit_task_id,
            review_handoff_event_id=review_handoff_event_id,
...
def _derive_factory_build_gate(
    *,
    ...
    b2 (REV2 — TOTAL entry): the creator profile is a factory profile → gate
        forced 1 UNCONDITIONALLY. The title/body conjunction is dropped: gate
        entry is total on the authenticated creator identity (``created_by``,
        derived server-side from the session identity), so omitting every
        optional title/body marker can never yield gate=0 for an official
        factory admission.
    ...
    b2 = (created_by or "") in FACTORY_PROFILES
    if b1 or b2:
        return 1
...
def _find_factory_receipt_bytes(
    ...
    """Return ``(receipt_bytes, uploaded_by)`` for the task attachment whose
    on-disk blob hashes to ``receipt_sha``, or ``None``.

    REV2: the receipt MUST be a task attachment (the ``uploaded_by`` provenance
    surface). The ``<workspace_path>/receipt`` convention path from REV1 is
    dropped — a workspace file has no ``uploaded_by`` and is directly writable
    by the completing worker, which is exactly the forgery vector the Monarch
    F1 finding identifies. The ``uploaded_by`` value is returned so the caller
    can enforce that only a trusted kernel-executor identity may bind a receipt.
    """
...
def _validate_factory_receipt(
    ...
    """Authenticity validation of a parsed proof-kernel receipt (r1..r8).
    ...
    # r8 (REV2) — actor identity must come from a canonical provider/runtime
    # metadata source, never self-declared prose (mirrors the kernel's
    # BYPASS_SELF_DECLARED_ACTOR_IDENTITY). This is a structural anti-
    # impersonation readback check, not a re-computation of actor identity.
    action = receipt.get("action_receipt_ref")
    if not isinstance(action, dict):
        _fail("missing action_receipt_ref object")
    actor_identity_source = action.get("actor_identity_source")
    if not isinstance(actor_identity_source, str) or not actor_identity_source.strip():
        _fail("action_receipt_ref.actor_identity_source missing")
    if actor_identity_source.strip().lower() in FACTORY_SELF_DECLARED_ACTOR_SOURCES:
        _fail("action_receipt_ref.actor_identity_source is self-declared")
    if action.get("actor_role") not in FACTORY_ACTOR_ROLES:
...
def _aion_governance_source_dir() -> Optional[Path]:
    ...
    content-addressed frozen-install path for the approved PR #947 merge. The
...
def complete_task(
    ...
    *,
    ...
    # Gate: factory-build terminal receipt (AION-889 Phase B REV2 —
    # provenance-bound authenticity, not shape). A factory-build task
    # (factory_build_gate=1) may not reach a terminal state until a proof-kernel
    # OUTCOME_ACCEPTED receipt is bound AND authenticated: the bound
    # ``factory_terminal_receipt_sha256`` must be the sha256 of a readable
    # receipt TASK ATTACHMENT whose ``uploaded_by`` is a trusted kernel-executor
    # identity (not the completing worker), and whose JSON passes r1..r8.
    ...
    # change. No C1-C10 re-computation happens here — the aion-governance proof
    # kernel stays the single semantic authority; this guard only enforces
    ...
    process_hygiene = _cleanup_workspace_on_completion(
...
def _derive_shared_dir_owned_pids(
    ...
    """Resolve the exact run's owned worker lineage for a shared-dir task.

    Ownership is bound to the exact ``run_id`` — the run being blocked or
    completed — never "the most recent ``spawned`` event task-wide". A task
    is retried across multiple runs, and a bare ``ORDER BY id DESC LIMIT 1``
    can pick a stale spawn identity from an older run (or one of several
    distinct identities on the same run), which is exactly the shared-dir
    collateral-signalling defect.

    The canonical ownership source is the ``spawned`` event(s) whose
    ``run_id`` matches *run_id*, each payload carrying ``pid`` + ``/proc``
    ``starttime`` (written by :func:`_set_worker_pid`). Both fields are
    required to prove ownership; descendants are re-discovered via
    ...
    if run_id is None:
        ...
        # shared directory. Never fall back to task-wide spawn history.
    ...
    spawn_pid, spawn_starttime = identities[0]
...
def _cleanup_workspace_on_completion(
    ...
    any error is logged and swallowed so completion is never blocked, but a
    deterministic evidence dict is returned so callers/tests can distinguish
    success, safe refusal, identity mismatch, and internal error.

    **AION-RL2-CORE-01-R17 repair:** the previous AION-CORE-PR6
    implementation derived ``owned_pids`` for ``workspace_kind=dir`` from a
    ``task_spawns`` table that does not exist in the canonical schema, so
    every dir-workspace completion raised ``sqlite3.OperationalError: no such
    table: task_spawns`` inside the try/except and silently skipped all
    process closure. This repair derives task-owned PID/starttime identity
    from the canonical ``task_events`` ``spawned`` event — the same source
    :func:`_cleanup_workspace` and :func:`_set_worker_pid` already use — and
    never queries a nonexistent table.

    **Fail-closed shared-dir ownership** (mirrors :func:`_cleanup_workspace`):

    - Only the ``spawned`` event(s) bound to the exact *run_id* with BOTH a
      valid ``pid`` AND a well-formed ``/proc`` ``starttime`` can prove
      ownership. Missing / legacy / malformed / unreadable starttime, a run
      with no spawn identity, or two distinct spawn identities on the same
      run → safe refusal (no signals).
    - Descendants are discovered via :func:`_discover_descendant_pids` with
      ``expected_starttime``, so a recycled PID is rejected as unowned.
    - There is no bare-PID fallback that can authorize shared-dir signals.

    Returns a dict with:
        - ``outcome``: one of ``"success"``, ``"safe_refusal"``,
          ``"identity_mismatch"``, ``"no_task"``, ``"no_workspace"``, or
          ``"internal_error"``.
        - ``task_id``, ``kind``, ``workspace`` (when known).
        - ``reason``: short reason for safe_refusal / identity_mismatch.
        - ``evidence``: the raw :func:`close_workspace_processes` dict when
          process closure ran.
        - ``error``: exception text on ``internal_error``.
    """
    try:
        row = conn.execute(
        ...
        if row is None:
            return {"outcome": "no_task", "task_id": task_id}
        wp = row["workspace_path"]
        wk = row["workspace_kind"]
        if not wp:
            return {"outcome": "no_workspace", "task_id": task_id, "kind": wk}
        ...
        owned_pids: set[int] | None = None
        if wk == "dir":
            owned_pids, _refusal = _derive_shared_dir_owned_pids(
                conn, task_id, run_id=run_id,
            )
        ...
        if evidence.get("signalled", 0) > 0:
            outcome = "success"
        elif evidence.get("skipped_identity_mismatch", 0) > 0:
        ...
        elif outcome == "safe_refusal" and wk == "dir":
            result["reason"] = (
                "unowned_processes_preserved"
                if evidence.get("skipped_unowned", 0)
                else "no_eligible_processes"
            )
        return result
    except Exception as exc:
        _log.warning(
